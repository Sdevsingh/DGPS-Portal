...
export default async function ClientPortalPage({
  ...
  const { data: tenantData } = await supabaseAdmin.from("tenants").select("id, name, slug").eq("id", tenantId).single();
  ...
  const jobs = (jobsRaw ?? []).map(formatJob).filter(
    (j) => j.agentEmail?.toLowerCase() === email?.toLowerCase() || j.createdByUserId === userId
  );
  ...
  const summaryCards: Array<{ view: ClientView; title: string; value: number; accent: string; border: string }> = [
            ...
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
              <div>
                <p className="inline-flex items-center rounded-full bg-white/20 px-2.5 py-1 text-[11px] font-semibold tracking-wide uppercase">
                  {tenantData?.name ?? session.user.tenantName}
                </p>
                <p className="text-blue-100 text-sm mt-3">Welcome back</p>
                <h1 className="text-2xl md:text-3xl font-bold">{session.user.name}</h1>
                <p className="text-blue-100 text-sm mt-1">{jobs.length} service request{jobs.length !== 1 ? "s" : ""}</p>
              </div>
              {tenantData?.slug && (
                <Link href={`/request/${tenantData.slug}`} className="inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-white text-blue-700 hover:bg-blue-50 rounded-xl text-sm font-semibold transition-colors shadow-sm shrink-0">
      ...
      <div className="px-4 py-6 max-w-3xl mx-auto space-y-4">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {summaryCards.map((card) => {
            ...
            return (
              <Link key={card.view} href={getViewHref(card.view)} className={`rounded-2xl border p-3 transition-all ${isActiveFilter ? `bg-white ${card.border} shadow-sm ring-1 ring-blue-200` : "bg-white border-gray-200 hover:border-blue-200 hover:shadow-sm"}`}>
                <p className="text-xs text-gray-500">{card.title}</p>
                <p className={`text-2xl font-bold mt-0.5 ${card.accent}`}>{card.value}</p>
                <p className="text-[11px] text-gray-400 mt-1">{isActiveFilter ? "Filtering now" : "Click to filter"}</p>
              </Link>
            );
          })}
        </div>
        ...
        <div className="flex items-center justify-between">
          <p className="text-sm text-gray-500">Showing <span className="font-semibold text-gray-800">{currentView === "all" ? "All Jobs" : summaryCards.find((c) => c.view === currentView)?.title}</span></p>
          {currentView !== "all" && <Link href="/client" className="text-sm font-medium text-blue-600 hover:text-blue-700">Clear Filter</Link>}
        </div>
        ...
        {filteredJobs.map((job) => {
          ...
          return (
            <Link key={job.id} href={`/client/jobs/${job.id}`} className="block bg-white rounded-2xl border border-gray-200 p-5 hover:shadow-md transition-all group">
              <div className="flex items-start justify-between mb-3">
                <div>
                  <p className="font-bold text-gray-900">{job.propertyAddress}</p>
                  <p className="text-sm text-gray-500 mt-0.5 flex items-center gap-2">
                    {job.category} · {job.jobNumber}
                    <span className={`text-[11px] px-2 py-0.5 rounded-full font-medium ${PRIORITY_STYLE[job.priority] ?? "bg-gray-100 text-gray-600"}`}>{job.priority || "medium"} priority</span>
                  </p>
                </div>
                {hasQuote && job.quoteStatus === "sent" && <span className="text-xs bg-blue-100 text-blue-700 px-2.5 py-1 rounded-full font-medium whitespace-nowrap">Quote Ready</span>}
                ...
                {!hasQuote && <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${STATUS_STYLE[normalizedStatus] ?? "bg-gray-100 text-gray-600"}`}>{(normalizedStatus || "new").replace(/_/g, " ")}</span>}
              </div>
              <div className="flex items-center justify-between">
                <p className="text-xs text-gray-500">Status: <span className="font-semibold text-gray-800 capitalize">{(normalizedStatus || "new").replace(/_/g, " ")}</span></p>
                <p className="text-xs text-gray-400">{new Date(job.createdAt).toLocaleDateString("en-AU", { day: "numeric", month: "short" })}</p>
              </div>
              <div className="flex justify-between mt-2">
                <p className="text-xs text-gray-400">Job updates available in chat</p>
                {hasQuote && <p className="text-sm font-bold text-gray-900">${Number(job.quoteTotalWithGst).toFixed(2)} <span className="text-xs font-normal text-gray-400">incl. GST</span></p>}
              </div>
              {job.quoteStatus === "sent" && <div className="mt-3 p-3 bg-blue-50 rounded-xl text-sm text-blue-700 font-medium">Your quote is ready — tap to review and approve</div>}
        ...
        {filteredJobs.length === 0 && (
          <div className="text-center py-16">
            ...
            <p className="text-gray-400 text-lg font-medium">{currentView === "all" ? "No service requests yet" : "No jobs in this view"}</p>
          </div>
        )}
      </div>
    </div>
